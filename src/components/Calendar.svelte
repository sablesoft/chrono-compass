<script lang="ts">
  ...
  import { DISPLAY_OPTIONS, EVENT_CORNERS, readDisplayOptions, saveDisplayOptions, resolveCalendarLayers } from '../lib/calendar/layers';
  ...
  const docs = useDocs(() => 'calendars/epoch-calendar.md', { getTitle: () => 'Epoch Calendar' });
  ...
  $: count = page ? (page.moon === 14 ? correctionDays(page) : 28) : 0;
  $: days = page ? Array.from({ length: count }, (_, i) => ({
  ...
  $: title = page?.moon === 14 ? (count === 1 ? 'Free Day' : 'Free Circle') : `Month ${page?.moon}`;
  ...
  function navigate(direction: -1 | 1) {
    try { if (page) selectPage(movePage(page, direction)); }
  ...
  function civilLabel(absolute: number, full = false): string {
    ...
    return `${d.day} ${months[d.month - 1]}${full ? ` ${y}` : ''}`;
  }
  function changeDisplayOptions(next: string[]) {
    displayOptions = next;
    try { saveDisplayOptions(localStorage, next); } catch {}
  }
  onMount(() => {
    try { displayOptions = readDisplayOptions(localStorage); } catch {}
    const timer = setInterval(() => clock = Date.now(), 1000);
    return () => clearInterval(timer);
  });
</script>

<section class="calendar" aria-label="Epoch Calendar">
  <div class="heading">
    <div><div class="calendarTitle"><p class="eyebrow">EPOCH CALENDAR</p><button class="infoButton" type="button" aria-label="About Epoch Calendar" title="About Epoch Calendar" on:click={docs.openDocs}>i</button></div><h1 aria-live="polite">{title}</h1></div>
    <div class="displayOptions"><DropdownButton label="Display options" items={DISPLAY_OPTIONS} value={displayOptions} onChange={changeDisplayOptions} buttonClass="calendarOptionsButton" /></div>
    <button class="todayButton" on:click={() => selectPage(today)}>Today</button>
  </div>
  {#if page}
    <p class="coordinate">GC {page.gc} / A {page.phrase}({PHRASES[page.phrase - 1]}) /
      {page.wave === 0 ? 'X' : `W ${page.wave}`} / Y {page.year}</p>
    <form class="selectors" on:submit|preventDefault={applySelection}>
    ...
    <div class="navigation">
      <button aria-label="Previous period" on:click={() => navigate(-1)}>←</button>
      <div class="periodMeta"><span>{page.moon === 14 ? `${count} ${count === 1 ? 'day' : 'days'}` : `${page.moon} / 13`}</span>
        {#if showGregorian && days.length}<small>{civilLabel(days[0].absolute, true)} — {civilLabel(days[days.length - 1].absolute, true)}</small>{/if}</div>
      <button aria-label="Next period" on:click={() => navigate(1)}>→</button>
    </div>
    {#if page.moon === 14}
    ...
    {:else}
      <div class="days" aria-label={title}>
        {#each days as day}
          <div class="day" class:today={day.absolute === todayDay} style:--day-color={`var(--calendar-${['red', 'white', 'blue', 'gold'][Math.floor((day.number - 1) / 7)]})`} aria-current={day.absolute === todayDay ? 'date' : undefined} title={day.address}>
            <span class="dayNumber">{day.number}</span>
            {#if showGregorian}<span class="civil">{civilLabel(day.absolute)}</span>{/if}
