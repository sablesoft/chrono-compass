...
export const horizon = {
    ...
    ready: true,
    info: [
        { defaultLabel: 'altitude', metaField: 'altitudeDeg', format: 'deg', enabled: false, spokes: '*', modal: 'Altitude is the target height relative to the local horizon. Positive values mean the body is above the horizon, negative values mean it is below.' },
        { defaultLabel: 'azimuth', metaField: 'azimuthDeg', format: 'deg', enabled: false, spokes: '*', modal: 'Azimuth is the horizontal direction along the horizon, measured clockwise from true north: N=0°, E=90°, S=180°, W=270°.' },
        ...
        { defaultLabel: 'max altitude', enabled: false, spokes: ['N'], modal: 'Maximum altitude is the highest point of the target above the horizon within the current horizon cycle.' },
        { defaultLabel: 'min altitude', enabled: false, spokes: ['S'], modal: 'Minimum altitude is the lowest point of the target below the horizon within the current horizon cycle.' },
        { defaultLabel: 'zenith', spokes: ['N'], modal: 'Zenith sector marks the local top-side arc of the cycle where altitude reaches its peak behavior.' },
        { defaultLabel: 'nadir', spokes: ['S'], modal: 'Nadir sector marks the local bottom-side arc of the cycle where altitude reaches its minimum behavior.' },
        { defaultLabel: 'above horizon', enabled: false, spokes: ['ENE', 'NE', 'NNE', 'N', 'NNW', 'NW', 'WNW'], modal: 'These spokes belong to the interval where the target remains above the local horizon.' },
        { defaultLabel: 'below horizon', enabled: false, spokes: ['WSW', 'SW', 'SSW', 'S', 'SSE', 'SE', 'ESE'], modal: 'These spokes belong to the interval where the target remains below the local horizon.' },
    ],
    ui: {
      'target': 'pointer',
    },
