...
Select a year and press **Go**, then **Print / Export year**. The preview uses the displayed Epoch year, your enabled **Display options**, and your location's time zone. Choose English, Russian or Portuguese, then **Print / Save as PDF**. The selected print language is remembered independently of the general documentation language. Generation runs on your device, including offline once the app's resources have been cached. Gregorian Calendar export is not included. The document title, and therefore the default PDF filename offered by the browser, contains the year code and language, for example `Harmonic Calendar - W13-Y5 - RU`.

The printable cover uses the full-bleed Chrono Compass artwork, with only the localized **Harmonic Calendar** title and the selected year's complete coordinate added dynamically. The site address is part of the fixed artwork; the time zone, Free Days placement and Dreamspell year name are omitted from the cover. Its reverse is blank. The calendar always uses **A4 landscape**. Each period has its calendar on the front and its legend on the back. Month legends include concise selected Gregorian and Dreamspell data plus enabled astronomical layers; repeated grid instructions, week-action lists, date-format instructions and chakra lists are omitted. Season and Earth–Sun Bind events have short explanations below the event line in the selected print language. Season wording follows the selected location's hemisphere; lunar phases need no extra explanation. Display options can also control future explanatory material, including dedicated chakra pages.

The Free period is drawn as a circle, like the application: a light-green circle for one **Free Day**, or four colored sectors with a light-green ring for four **Free Days**. **At end** is the default and uses the Free Day(s) immediately after Month 13, opening the following Epoch year. **At start** uses the different Free Day(s) immediately before Month 1. **At both ends** includes both periods. Their dates and one-day/four-day lengths are calculated independently, preserving continuity with Gregorian dates. The first two modes produce **15 sheets / 30 sides**; both ends produces **16 sheets / 32 sides**. The reverse explains only that the correction day(s) lie outside the calendar grid and between adjacent calendar years.

There is **24 mm of clear space for rings** at the top of fronts and bottom of backs. Print double-sided with **flip on long edge**, at **100%**, without added margins or browser headers/footers. Preserve the blank cover reverse. Test one sheet first: lifting it around the top rings should reveal an upright legend. The browser/printer may override paper settings, so verify A4 landscape in its dialog. Choose **Save as PDF** there to download a document. If a calendar or legend side exceeds its sheet, printing is blocked with a message rather than losing text.

The localized [Harmonic Calendar print supplement](harmonic-calendar-print-supplement.md) is appended automatically after the dated calendar. Its title and contents begin the guide, and each of the six main sections starts on the front of a new physical sheet. Longer sections continue naturally onto following sides. If a section ends on the front, the reverse of that sheet remains blank so the next section can begin on a new front. Supplement pages follow the same duplex binding geometry: 24 mm at the top on front pages and at the bottom on reverse pages.
