...
`calendar/print.ts` builds paired sheets using the existing core, layer registry, event markers and symbolic constants. Each of 13 months and the separate correction period has a grid front and a localized legend back. All enabled display options are represented, including Gregorian dates, Dreamspell names, chakras and every registered astronomical layer. Unsupported layer ranges and empty event lists remain explicit. Times use the selected location zone and existing approximation conventions. No new holiday calendar is implied by “special days”: these are the existing correction days and enabled layer events.

The cover has a deliberately blank reverse: 15 physical sheets / 30 PDF pages in total. Order is cover front, blank back, then alternating period front/back. **Free Days at end** is the default; **at start** places them immediately after the cover. This is paper collation only: the correction period still belongs chronologically to the beginning of the selected year. Do not substitute the next year's correction period.
