...
Future supplementary documents use `PrintAppendix` pairs of plain-text front/back content. Missing backs are padded with a blank page, preserving parity. Appendices must fit a sheet (or be split into multiple explicit pairs); the same overflow check applies. These are not yet exposed as a user-facing document picker. Enabled display options will determine which supplementary explanations are appended, including the future chakra sheet. The editorial source for the initial general supplement is maintained in [English](../public/docs/en/calendars/harmonic-calendar-print-supplement.md) and [Russian](../public/docs/ru/calendars/harmonic-calendar-print-supplement.md). It contains a table of contents and the sections About, Free Days, Number 4, Number 7, Number 13 and Practice; print pagination and option-specific inclusion remain future work.
